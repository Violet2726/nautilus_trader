...
fn test_ashare_cancel_order_denied_in_locked_phase(
    ...
    // 模拟 09:21 (PreAuctionLocked，UTC = 01:21)
    let ts_locked = UnixNanos::from((1 * 3600 + 21 * 60) as u64 * 1_000_000_000);
    ...
    let portfolio = Portfolio::new(cache.clone(), clock.clone(), None);
    let mut risk_engine = nautilus_risk::engine::RiskEngine::new(config, portfolio, clock, cache);
    ...
    assert!(
        reason.contains("CANCEL_DENIED"),
        "Expected CANCEL_DENIED in reason, got: {reason}"
    );
...
fn test_ashare_cancel_order_accepted_in_continuous_am(
    ...
    // 模拟 10:00 (ContinuousAm，UTC = 02:00)
    let ts_am = UnixNanos::from((2 * 3600) as u64 * 1_000_000_000);
    let clock = Rc::new(RefCell::new(TestClock::new()));
...
fn test_ashare_t1_sell_exceeds_sellable(strategy_id_ema_cross: StrategyId, trader_id: TraderId) {
    ...
    let instrument = get_ashare_instrument();
    let my_account_id = AccountId::from("XSHE-001");
    let account = cash_account(AccountState::new(
        my_account_id.clone(),
        AccountType::Cash,
        vec![AccountBalance::new(
            Money::from("1000000 CNY"),
            Money::from("0 CNY"),
            Money::from("1000000 CNY"),
        )],
        vec![],
    ...
    // 预加载 T+1 持仓：总持仓 1000，今日买入 500 (可卖 500)
    risk_engine.t1_load_position(my_account_id.clone(), instrument.id(), 1000.0, 500.0);

    // 发起卖出 600 的订单
    let order_sell = OrderTestBuilder::new(OrderType::Limit)
    ...
    let submit_sell = SubmitOrder::from_order(
        &order_sell,
        trader_id,
        None,
        None,
