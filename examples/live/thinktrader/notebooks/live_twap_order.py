...
class TWAPOrderStrategyConfig(StrategyConfig, frozen=True):
...
class TWAPOrderStrategy(Strategy):
    """
    单次 TWAP 下单的简单测试策略
    在收到该股票的第一个 QuoteTick 后，发起一个 TWAP 买单。
    """

    def __init__(self, config: TWAPOrderStrategyConfig) -> None:
    ...
    def on_start(self) -> None:
        ...
        self.log.info(f"订阅行情以触发 TWAP 下单: {self.instrument_id}")
    ...
    def place_twap_order(self) -> None:
        ...
        self.log.info(
            f"正在提交 TWAP 订单: 标的={self.instrument_id}, 数量={self.trade_size}, "
...
config_node = TradingNodeConfig(
    trader_id=TraderId("TESTER-TWAP"),
...
if __name__ == "__main__":
    ...
    strategy_config = TWAPOrderStrategyConfig(
        instrument_id=instrument_id,
        trade_size=Decimal(200),      # 总数量
        horizon_secs=20.0,             # 20 秒完成
        interval_secs=10.0             # 每隔 10 秒下一次单
    )

    strategy = TWAPOrderStrategy(config=strategy_config)
    node.trader.add_strategy(strategy)

    # 注册 TWAP 执行算法
    # 注意：Rust pyo3 版 TwapAlgorithm 当前无法在 live 环境中直接注册，
    # 因为 Trader.add_exec_algorithm() 传入的 cache/clock 是 Cython 对象，
    # 而 Rust 侧的 ExecutionAlgorithmCore.register() 需要 Rc<RefCell<Cache>>
    # (Rust 原生类型)，两种类型系统之间的桥接尚未完成。
    # 因此这里使用 Cython 实现的 TWAPExecAlgorithm（算法逻辑完全一致）。
    twap_algo = TWAPExecAlgorithm(config=TWAPExecAlgorithmConfig())
    node.trader.add_exec_algorithm(twap_algo)

    node.build()

    print(f"正在启动 TWAP 测试策略, 标的: {instrument_id}...")
